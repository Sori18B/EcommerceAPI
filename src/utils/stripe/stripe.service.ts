...
export class StripeService {
    ...
    // Actualizar cliente
    async updateCustomer(customerId: string, data: Partial<Stripe.CustomerUpdateParams>) {
        try {
            return await this.stripe.customers.update(customerId, data);
        } catch (error) {
            throw new Error(`Error al actualizar cliente en Stripe: ${error.message}`);
        }
    }
    ...
    async createPaymentMethod(customerId: string, paymentMethodData: Stripe.PaymentMethodCreateParams) {
        try {
